...
class fish2:
    ...
    def run(frame):
        # print('Run 'fish2)
        try:
          MyFishLib.MyFishLib_rt.fish2_run_rt(frame)
        except Exception as e:
          if not hasattr(frame, 'errOnce'):
            print('MyFishLib.MyFishLib_rt.fish2_run_rt failed\n', str(e))
            import traceback
    ...
    codeDescr =        (
        ("loaderStream",
          (
            ...
            ("Code","frame.vars.tgtPos[0][0]= (random() - .5) * 20 #< your code goes here>"),
            ("Code","frame.vars.tgtPos[0][1]= (random() - .5) * 20 #< your code goes here>"),
            ("Label","RunLoop"),
            ("CodeBlock",'''
#quat = Quat()
...
'''),
    ...
    def start(stack):
        return MyFishLib.MyFishLib_rt.leaderFish2_start_rt(stack)
    ...
    def run(frame):
        ...
        try:
          ...
          if not hasattr(frame, 'errOnce'):
            print('MyFishLib leaderFish2 runParallel failed\n', str(e))
            import traceback
            traceback.print_exception(e)
            frame.errOnce = True
