...
class MyTestFishLib:
    def build():
        WyeCore.Utils.buildLib(MyTestFishLib)
    canSave = True  # all verbs can be saved with the library
    class MyTestFishLib_rt:
     pass #1
    ...
    frame.vars.posAngle[0] = angle
        ...
        def build():
            ...
            return WyeCore.Utils.buildParallelText('MyTestFishLib','testParallelFish', MyTestFishLib.testParallelFish.codeDescr, MyTestFishLib.testParallelFish)
    
        def start(stack):
            return MyTestFishLib.MyTestFishLib_rt.testParallelFish_start_rt(stack)
        ...
        def run(frame):
            ...
            try:
              ...
              if not hasattr(frame, 'errOnce'):
                print('MyTestFishLib testParallelFish runParallel failed\n', str(e))
                import traceback
