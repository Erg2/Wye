...
class WyeUI(Wye.staticObj):
    ...
    # put up popup dialog box with given color (see Wye.color.NORMAL_COLOR, WARNING_COLOR, ERROR_COLOR
    dummy = [0]
    def doPopUpDialog(titleText, mainText, color=Wye.color.NORMAL_COLOR):
        ...
        stk = []
        dlgFrm = WyeCore.libs.WyeUI.Dialog.start(stk)
        stk.append(dlgFrm)
        dlgFrm.params.retVal = WyeCore.libs.WyeUI.dummy
        dlgFrm.params.title = [titleText]
        point = NodePath("point")
        point.reparentTo(render)
        point.setPos(base.camera, (-1.5, 7, 1))
        pos = point.getPos()
        point.removeNode()
        dlgFrm.params.position = [(pos[0], pos[1], pos[2]), ]
        dlgFrm.params.parent = [None]
        WyeCore.libs.WyeUI.dummy[0] = dlgFrm
    ...
    class InputInteger(InputText):
        ...
        def doSelect(frame, dlgFrame, tag):
            if tag in frame.vars.tags[0]:
                ...
                inWidg.setColor(Wye.color.SELECTED_COLOR)  # set input background to "has focus" color
                WyeUI.Dialog.drawCursor(frame)
                WyeUI.Dialog._activeInputInteger = frame
                return True
            else:
                return False
        ...
        def doSelect(frame, dlgFrame, tag):
            if tag in frame.vars.tags[0]:
                ...
                if frame.vars.clickCount[0] <= 0:  # if not in an upclick count, process click
                    ...
                    if frame.params.callback:
                        # user's param callback
                        # print("InputButton doCallback")
                        WyeCore.libs.WyeUI.Dialog.doCallback(dlgFrame, frame, tag)
                return True
            else:
                return False
    ...
    class InputDropdown:
        ...
        def display(frame, dlgFrm, pos):
            ...
            # update dialog pos to next free space downward
            pos[2] -= btn.getHeight()  # update to next position
            frame.vars.size[0] = (width + .5, 0, max(lbl.getHeight(), btn.getHeight()))
        ...
        def doSelect(frame, dlgFrame, tag):
            if tag in frame.vars.tags[0]:
                ...
                if frame.params.callback:
                    # user's param callback
                    #print("InputDropdown doSelect: doCallback", frame.params.callback[0].__name__)
                    WyeCore.libs.WyeUI.Dialog.doCallback(dlgFrame, frame, tag)
                return True
            else:
                return False
                ...
                match (frame.PC):
                    ...
                    case 1:
                        ...
                        if dlgFrm.status == Wye.status.SUCCESS:
                            rowFrm.verb.setValue(rowFrm, dlgFrm.params.retVal[0])
    ...
    class Dialog:
        ...
        varDescr = (("position", Wye.dType.FLOAT_LIST, (0,0,0)),            # pos copy  *** REQUIRED ***
                    ...
                    ("currInp", Wye.dType.OBJECT, None),                    # index to current focus widget, if any
            ...
            match frame.PC:
                case 0:     # Start up case - set up all the fields
                    ...
                    if parent is None:
                        ...
                        #dlgHeader = WyeUI._3dText(text=frame.params.title[0], color=(Wye.color.HEADER_COLOR), pos=frame.params.position[0], scale=(1,1,1))
                    ...
                    txt = WyeUI._3dText("OK", color=(Wye.color.CONTROL_COLOR), pos=tuple(pos), scale=(1, 1, 1),
                    ...
                    txt = WyeUI._3dText("Cancel", color=(Wye.color.CONTROL_COLOR), pos=tuple(pos), scale=(1, 1, 1),
                                        parent=dlgHeader.getNodePath())
                    frame.vars.dlgWidgets[0].append(txt)
        ...
        def doCallback(frame, inFrm, tag):
            ...
            if hasattr(inFrm.vars, "callback"):
                callVerb = inFrm.vars.callback[0]
                #print("Dialog doCallback: vars callback", callVerb.__name__)
            elif hasattr(inFrm.params, "callback"):
                callVerb = inFrm.params.callback[0]
                #print("Dialog doCallback: params callback", callVerb.__name__)
            if callVerb:
                # start the verb
                verbFrm = callVerb.start(frame.SP)
                # handle user data
                if hasattr(inFrm.vars, "optData"):      # check vars copy first to handle InputXxxx classes
                    if len(inFrm.vars.optData) > 0:
                        data = inFrm.vars.optData[0]
                    else:
                        data = None
                    #print("   vars optData", data)
                else:
                    if hasattr(inFrm.params, "optData"):
        ...
        def doSelect(frame, tag):
            ...
            if not prevSel is None and prevSel != frame.vars.currInp[0] and not closing:
                inFrm =prevSel
                if inFrm.verb in [WyeUI.InputText, WyeUI.InputInteger]:
                    inWidg = inFrm.vars.gWidget[0]
        ...
        def run(frame):
            match frame.PC:
                ...
                case 1:
                    ...
                    if not frame.vars.currInp[0] is None:
                        frame.params.retVal[0] = -1
                        ix = 0
    ...
    class MainMenuDialog:
        ...
        def run(frame):
            match(frame.PC):
                case 0:
                    ...
                    verChkFrm = WyeCore.libs.WyeUI.InputCheckbox.start(dlgFrm.SP)
                    dlgFrm.params.inputs[0].append([verChkFrm])
                    verChkFrm.params.frame = [None]
                    verChkFrm.params.parent = [None]
                    verChkFrm.params.value = [True]
                    verChkFrm.params.label = ["Show Wye Version"]
                    verChkFrm.params.callback = [WyeCore.libs.WyeUI.MainMenuDialog.VerCheckCallback]  # button callback
                    ...
                    btnFrm.params.callback = [WyeCore.libs.WyeUI.MainMenuDialog.TestCreatLibCallback]  # button callback
                    #btnFrm.params.optData = [(attrIx, btnFrm, dlgFrm, verb)]  # button row, dialog frame
                    WyeCore.libs.WyeUI.InputButton.run(btnFrm)
                    ...
                    midInsFrm = WyeCore.libs.WyeUI.InputInteger.start(dlgFrm.SP)
                    ...
                    midNoteFrm = WyeCore.libs.WyeUI.InputInteger.start(dlgFrm.SP)
                    ...
                    midVolFrm = WyeCore.libs.WyeUI.InputInteger.start(dlgFrm.SP)
                    ...
                    midLenFrm = WyeCore.libs.WyeUI.InputText.start(dlgFrm.SP)
        ...
        class VerCheckCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.MainMenuDialog.VerCheckCallback, stack)
        ...
        class TestButtonCallback:
            ...
            def run(frame):
                print("Test Button")
                WyeCore.libs.WyeUI.doPopUpDialog("Test Callback", "Pop Up Dialog Test")
                WyeCore.libs.WyeUI.doPopUpDialog("Test Callback", "Pop Up Dialog Test n", Wye.color.NORMAL_COLOR)
                WyeCore.libs.WyeUI.doPopUpDialog("Test Callback", "Pop Up Dialog Test w", Wye.color.WARNING_COLOR)
                WyeCore.libs.WyeUI.doPopUpDialog("Test Callback", "Pop Up Dialog Test e", Wye.color.ERROR_COLOR)
    ...
    class EditVerb:
            ...
            match(frame.PC):
                case 0:
                    ...
                    if not hasattr(verb, "codeDescr"):
                        ...
                        # take self off active object list
                        WyeCore.World.stopActiveObject(frame)
                        WyeCore.libs.WyeUI.doPopUpDialog("Not Editable", "Cannot edit hard-coded verb", Wye.color.WARNING_COLOR)
                    ...
                    if len(verb.paramDescr) > 0:     # if we have params, list them
                        ...
                        for param in verb.paramDescr:
                            ...
                            btnFrm.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditParamCallback]  # button callback
                            btnFrm.params.optData = [(attrIx, btnFrm, dlgFrm, frame)]  # button row, dialog frame
                            WyeCore.libs.WyeUI.InputButton.run(btnFrm)
                    ...
                    if len(verb.varDescr[0]) > 0:
                        ...
                        for var in verb.varDescr:
                            ...
                            btnFrm.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditVarCallback]  # button callback
                            btnFrm.params.optData = [(attrIx, btnFrm, dlgFrm, frame)]  # button row, dialog frame
                            WyeCore.libs.WyeUI.InputButton.run(btnFrm)
                        ...
                        if len(verb.codeDescr) > 0:
                            ...
                            # regular boring normal single stream code
                                ...
                                for tuple in verb.codeDescr:
                                    WyeCore.libs.WyeUI.EditVerb.bldEditCodeLine(tuple, level, frame, attrIx, row, dlgFrm)

                                    attrIx += 1
                        else:
                            # line edit button
                            editLnFrm = WyeCore.libs.WyeUI.InputButton.start(dlgFrm.SP)
                            dlgFrm.params.inputs[0].append([editLnFrm])
                            editLnFrm.params.label = ["+/-"]
                            editLnFrm.params.callback = [
                                WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback]  # button callback
                            # note: optData further down so can pass btnFrm
                            editLnFrm.params.frame = [None]
                            editLnFrm.params.parent = [None]

                            lblFrm = WyeCore.libs.WyeUI.InputLabel.start(dlgFrm.SP)
                            lblFrm.params.layout = [Wye.layout.ADD_RIGHT]  # put after the line edit button
                            lblFrm.params.frame = [None]  # return value
                            lblFrm.params.parent = [None]
                            lblFrm.params.label = ["  <add code here>"]
                            WyeCore.libs.WyeUI.InputLabel.run(lblFrm)
                            dlgFrm.params.inputs[0].append([lblFrm])
                    ...
                    tstLblFrm.params.color = [Wye.color.SUBHD_COLOR]
                    ...
                    tstBtnFrm.params.callback = [WyeCore.libs.WyeUI.EditVerb.TestCodeCallback]  # button callback
        ...
        def bldEditCodeLine(tuple, level, editVerbFrm, attrIx, row, dlgFrm):
            ...
            dlgFrm.params.inputs[0].append([editLnFrm])
            editLnFrm.params.label = ["+/-"]
            editLnFrm.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback]  # button callback
            # note: optData further down so can pass btnFrm
            editLnFrm.params.frame = [None]
            editLnFrm.params.parent = [None]
            ...
            # fill in prev button opt data inlcuding ptr to this ctl
            editLnFrm.params.optData = [(editLnFrm, dlgFrm, editVerbFrm, tuple, btnFrm, level)]  # button row, dialog frame
            editLnFrm.verb.run(editLnFrm)
            ...
            if not tuple[0] is None and "." in tuple[0]:
                ...
                btnFrm.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditVerbCallback]  # button callback

                # display verb's params (if any)
                if len(tuple) > 1:
                    for paramTuple in tuple[1:]:
                        row[0] += 1
                        WyeCore.libs.WyeUI.EditVerb.bldEditCodeLine(paramTuple, level + 1, editVerbFrm, attrIx, row, dlgFrm)
            else:
                match tuple[0]:
                    case "Code" | None:  # raw Python
                        btnFrm.params.label = [indent + "Code: " + tuple[1]]
                        btnFrm.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditCodeCallback]  # button callback
                    ...
                    case "GoTo":
                        ...
                        btnFrm.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditSpecialCallback]  # button callback
            ...
            btnFrm.params.optData = [(btnFrm, dlgFrm, editVerbFrm, tuple, level, editLnFrm)]  # button row, dialog frame
        ...
        class EditVerbCallback:
            ...
            def start(stack):
                return Wye.codeFrame(WyeUI.EditVerb.EditVerbCallback, stack)
            ...
            def run(frame):
                ...
                match (frame.PC):
                    case 0:
                        ...
                        libFrm.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditVerbCallback.SelectLibCallback]
                        libFrm.params.optData = ((varIx, libFrm, dlgFrm, editVerbFrm, frame.vars.libName, verbFrm),)
                        ...
                        verbFrm.params.optData = ((varIx, verbFrm, dlgFrm, editVerbFrm, frame.vars.verbName),)
                        verbFrm.verb.run(verbFrm)
            ...
            class SelectLibCallback:
                ...
                def start(stack):
                    ...
                    return Wye.codeFrame(WyeUI.EditVerb.EditVerbCallback.SelectLibCallback, stack)
                ...
                def run(frame):
                    ...
                    if libNameLst[0] != newLib:
                        libNameLst[0] = newLib
                        verbLst = WyeCore.libs.WyeUI.EditVerb.EditVerbCallback.buildVerbList(libNameLst[0])
                        verbLst.append(WyeCore.libs.WyeUI.EditVerb.EditVerbCallback.noneSelected)
            ...
            class SelectVerbCallback:
                ...
                def start(stack):
                    ...
                    return Wye.codeFrame(WyeUI.EditVerb.EditVerbCallback.SelectVerbCallback, stack)
        ...
        class EditCodeLineCallback:
            ...
            def run(frame):
                ...
                match (frame.PC):
                    case 0:
                        # build code line add/sub/copy
                        dlgFrm = WyeCore.libs.WyeUI.Dialog.start([])

                        dlgFrm.params.retVal = frame.vars.dlgStat
                        dlgFrm.params.title = ["Modify Line"]
                        dlgFrm.params.parent = [parentFrm]
                        dlgFrm.params.position = [(.5, -.3, -.5 + btnFrm.vars.position[0][2]), ]

                        addUpBtn = WyeCore.libs.WyeUI.InputButton.start(dlgFrm.SP)
                        dlgFrm.params.inputs[0].append([addUpBtn])
                        addUpBtn.params.frame = [None]
                        addUpBtn.params.parent = [None]
                        addUpBtn.params.label = ["  Add line before"]
                        addUpBtn.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.EditModline]  # button callback
                        addUpBtn.params.optData = [(WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.ADD_UP, addUpBtn, parentFrm, editVerbFrm, level, editLnFrm, btnFrm)]  # button row, dialog frame
                        WyeCore.libs.WyeUI.InputButton.run(addUpBtn)

                        copyBtn = WyeCore.libs.WyeUI.InputButton.start(dlgFrm.SP)
                        dlgFrm.params.inputs[0].append([copyBtn])
                        copyBtn.params.frame = [None]
                        copyBtn.params.parent = [None]
                        copyBtn.params.label = ["  Copy line"]
                        copyBtn.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.EditModline]  # button callback
                        copyBtn.params.optData = [(WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.COPY, copyBtn, parentFrm, editVerbFrm, level, editLnFrm, btnFrm)]  # button row, dialog frame
                        WyeCore.libs.WyeUI.InputButton.run(copyBtn)

                        delBtn = WyeCore.libs.WyeUI.InputButton.start(dlgFrm.SP)
                        dlgFrm.params.inputs[0].append([delBtn])
                        delBtn.params.frame = [None]
                        delBtn.params.parent = [None]
                        delBtn.params.label = ["  Delete line"]
                        delBtn.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.EditModline]  # button callback
                        delBtn.params.optData = [(WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.DELETE, delBtn, parentFrm, editVerbFrm, level, editLnFrm, btnFrm)]  # button row, dialog frame
                        WyeCore.libs.WyeUI.InputButton.run(delBtn)

                        addDownBtn = WyeCore.libs.WyeUI.InputButton.start(dlgFrm.SP)
                        dlgFrm.params.inputs[0].append([addDownBtn])
                        addDownBtn.params.frame = [None]
                        addDownBtn.params.parent = [None]
                        addDownBtn.params.label = ["  Add line after"]
                        addDownBtn.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.EditModline]  # button callback
                        addDownBtn.params.optData = [(WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.ADD_DOWN, addDownBtn, parentFrm, editVerbFrm, level, editLnFrm, btnFrm)]  # button row, dialog frame
                        WyeCore.libs.WyeUI.InputButton.run(addDownBtn)
            ...
            class EditModline:
                ...
                def start(stack):
                    print("EditModline started")
                    return Wye.codeFrame(WyeUI.EditVerb.EditCodeLineCallback.EditModline, stack)

                def run(frame):
                    data = frame.eventData
                    op = data[1][0]
                    btnFrm = data[1][1]         # addUpBtn
                    parentFrm = data[1][2]
                    editVerbFrm = data[1][3]
                    level = data[1][4]
                    editLnFrm = data[1][5]
                    ...
                    #print("EditModLine btnFrm", btnFrm.params.label[0])
                    #print("            parentFrm", parentFrm.params.title[0])
                    #print("            editVerbFrm verb", editVerbFrm.params.verb[0].__name__)
                    #print("            level", level)
                    #print("            ediLnFrm", editLnFrm.params.label[0])
                    #print("            codeFrm", codeFrm.params.label[0])
                    ...
                    #print("EditModline run: data", data, " op", op)

                    match (op):
                        case WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.ADD_UP:
                            ...
                            tuple = codeFrm.params.optData[0][3]
                            ...
                            if parentList:
                                ...
                                print("EditModLine: failed to find tuple '", tuple," parent list.  Bad news")

                            # TODO - this is going to fail 'cause codeDescr is a const.
                            # Need to have an AST to modify!!!!!
                            editVerbFrm.vars.newCodeDescr[0].insert(ix, newLn)

                            # get position of current dlg row in this location
                            currPos = btnFrm.vars.position[0]
                            pos = [currPos[0], currPos[1], currPos[2]]
                            ...
                            # new line edit button
                            newEditLn = WyeCore.libs.WyeUI.InputButton.start(parentFrm.SP)
                            parentFrm.params.inputs[0].insert(ix, [newEditLn])  # insert
                            newEditLn.params.label = ["+/-"]
                            newEditLn.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback]  # button callback
                            # note: optData further down so can pass btnFrm
                            newEditLn.params.frame = [None]
                            newEditLn.params.parent = [None]
                            newEditLn.verb.run(newEditLn)
                            setattr(newEditLn, "parentFrame", parentFrm)

                            # new code line
                            newBtn = WyeCore.libs.WyeUI.InputButton.start(parentFrm.SP)
                            parentFrm.params.inputs[0].insert(ix+1, [newBtn])
                            newBtn.params.layout = [Wye.layout.ADD_RIGHT]
                            newBtn.params.frame = [None]
                            newBtn.params.parent = [None]
                            newBtn.params.label = [indent + "Verb: " + vStr]
                            newBtn.params.callback = [WyeCore.libs.WyeUI.EditVerb.EditVerbCallback]  # button callback
                            setattr(newBtn, "parentFrame", parentFrm)

                            # fill in callback data and run the display
                            # todo - figure out how to pass correct level
                            newEditLn.params.optData = [(newEditLn, parentFrm, editVerbFrm, tuple, btnFrm, level)]  # button row, dialog frame
                            newEditLn.verb.run(editLnFrm)
                            newEditLn.verb.display(newEditLn, parentFrm,LVecBase3f(0,0,0))  # position will be updated by redisplay, below

                            newBtn.params.optData = [(newBtn, parentFrm, editVerbFrm, tuple, level, editLnFrm)]  # button row, dialog frame
                            newBtn.verb.run(newEditLn)
                            newBtn.verb.display(newBtn, parentFrm,LVecBase3f(0,0,0))  # position will be updated by redisplay, below

                            # readjust the position for all the inputs
                            pos = LVecBase3f(0, 0, 0-parentFrm.vars.dragObj[0].getHeight())
                            nInputs = len(parentFrm.params.inputs[0])
                            ...
                            for ii in range(nInputs):
                                # print("  Dialog input", ii, " frame", frame.params.inputs[0][ii])
                                inFrm = parentFrm.params.inputs[0][ii][0]
                                # print("    inFrm", inFrm)
                                ...
                                if hasattr(inFrm.verb, "display"):
                                    if inFrm.params.layout[0] == Wye.layout.ADD_RIGHT:
                                        pos[0] = newX
                                        pos[2] = prevZ
                                        inFrm.verb.redisplay(inFrm, parentFrm, pos)  # displays label, updates pos
                                    else:
                                        prevZ = pos[2]
                                        pos[0] = 0
                                        inFrm.verb.redisplay(inFrm, parentFrm, pos)  # displays label, updates pos
                                    ...
                                    print("EditModline: Error. Unknown input verb", inFrm.verb.__name__)


                        case WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.COPY:
                            print("EditModline: Copy")
                            pass

                        case WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.DELETE:
                            print("EditModline: Delete")
                            pass

                        case WyeCore.libs.WyeUI.EditVerb.EditCodeLineCallback.ADD_DOWN:
                            print("EditModline: Add down")
                            ix = parentFrm.params.inputs[0].index(btnFrm)
                            parentFrm.params.inputs[0].insert(ix+1, newLn)
        ...
        class EditCodeCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.EditVerb.EditCodeCallback, stack)
                ...
                match (frame.PC):
                    case 0:
                        ...
                        # sequential verb
                            ...
                            # Code type
                            rowIx = -1
                            #print("***** input list. Look for", btnFrm.params.label, ", ", btnFrm, " in dialog", parentFrm.params.title[0])
                            # rowIx = parentFrm.params.inputs[0].index(btnFrm)
                            for ii in range(len(parentFrm.params.inputs[0])):
                                #print(" inp", ii, " ", parentFrm.params.inputs[0][ii][0].verb.__name__, " ", parentFrm.params.inputs[0][ii][0].params.label[0], " ", parentFrm.params.inputs[0][ii])
                                if parentFrm.params.inputs[0][ii][0] == btnFrm:
                                    #print("   match at", ii)
                                    rowIx = ii
                                    break

                            verbType = editVerbFrm.vars.newCodeDescr[0][rowIx][0]
                            if verbType is None:
                                verbType = "Code"
        ...
        class EditSpecialCallback:
            ...
            def start(stack):
                return Wye.codeFrame(WyeUI.EditVerb.EditSpecialCallback, stack)
        ...
        class EditParamCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.EditVerb.EditParamCallback, stack)
            ...
            def run(frame):
                ...
                match (frame.PC):
                    case 0:
                        ...
                        nParams = len(editVerbFrm.vars.newParamDescr)
                    ...
                    case 1:
                        ...
                        if dlgFrm.params.retVal[0] == Wye.status.SUCCESS:
                            ...
                            editVerbFrm.vars.newParamDescr[0].insert(paramIx, [paramIx, label, wType, accessCode])
...
#
        ...
        class EditParamTypeCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.EditVerb.EditParamTypeCallback, stack)
        ...
        class EditVarCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.EditVerb.EditVarCallback, stack)
            ...
            def run(frame):
                ...
                match (frame.PC):
                    case 0:
                        ...
                        nVars = len(editVerbFrm.vars.newVarDescr[0])
                        ...
                        # Var name
                        #print("EditVarCallback varIx", varIx, " newVarDescr", editVerbFrm.vars.newVarDescr[0])
                    ...
                    case 1:
                        ...
                        if dlgFrm.params.retVal[0] == Wye.status.SUCCESS:
                            ...
                            editVerbFrm.vars.newVarDescr[0].insert(varIx,[label, wType, initVal])
        ...
        class EditVarTypeCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.EditVerb.EditVarTypeCallback, stack)
        ...
        class TestCodeCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.EditVerb.TestCodeCallback, stack)
            ...
            def run(frame):
                ...
                btnFrm = data[1][0]
                parentFrm = data[1][1]
                label, wType, initVal = data[1][2]
                #print("param ix", data[1][0], " parentFrm", parentFrm.verb.__name__, " verb", label, wType, initVal.vars.oldVerb[0].__name__)
    ...
    class ObjectDebugger:
        ...
        def run(frame):
            ...
            match (frame.PC):
                case 0:
                    ...
                    if len(paramDescr) > 0:     # if we have params, list them
                        ...
                        for param in paramDescr:
                            ...
                            btnFrm.params.callback = [WyeCore.libs.WyeUI.ObjectDebugger.DebugParamCallback]  # button callback
                    ...
                    if len(varDescr) > 0:       # if we have variables, list them
                        ...
                        for var in varDescr:
                            ...
                            btnFrm.params.callback = [WyeCore.libs.WyeUI.ObjectDebugger.DebugVarCallback]  # button callback
                            btnFrm.params.optData = [(attrIx, btnFrm, dlgFrm, objFrm, frame)]  # button row, dialog frame
                            WyeCore.libs.WyeUI.InputButton.run(btnFrm)
                        ...
                        runChkFrm.params.callback = [WyeCore.libs.WyeUI.ObjectDebugger.RunCallback]  # button callback
                        ...
                        btnFrm.params.callback = [WyeCore.libs.WyeUI.ObjectDebugger.DebugRefreshVarCallback]  # button callback
                        btnFrm.params.optData = [(attrIx, btnFrm, dlgFrm, objFrm, frame)]  # button row, dialog frame
                        btnFrm.params.color = [(1, 1, 0, 1)]
                        WyeCore.libs.WyeUI.InputButton.run(btnFrm)
                        ...
                        if hasattr(oFrm.verb, "codeDescr"):
                            ...
                            for tuple in codeDescr:
                                ...
                                if tuple[0] is None:
                                    ...
                                    btnFrm.params.callback = [WyeCore.libs.WyeUI.ObjectDebugger.DebugVerbCallback]  # button callback
                                else:
                                    match tuple[0]:
                                        case "Code":         # raw Python
                                            btnFrm.params.label = ["  Code: " + tuple[1]]
                                            btnFrm.params.callback = [WyeCore.libs.WyeUI.ObjectDebugger.DebugCodeCallback]  # button callback
                                        case "CodeBlock":    # multi-line raw Python
                                        ...
                                        case "GoTo":
                                            ...
                                            btnFrm.params.callback = [WyeCore.libs.WyeUI.ObjectDebugger.DebugSpecialCallback]  # button callback
                ...
                case 1:
                    ...
                    frame.status = Wye.status.SUCCESS  # done
                    WyeUI.ObjectDebugger.activeObjs.pop(objFrm)
        ...
        class DebugParamCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.ObjectDebugger.DebugParamCallback, stack)
        ...
        class DebugVarCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.ObjectDebugger.DebugVarCallback, stack)
        ...
        class DebugVerbCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.ObjectDebugger.DebugVerbCallback, stack)
        ...
        class DebugCodeCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.ObjectDebugger.DebugCodeCallback, stack)
        ...
        class DebugSpecialCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.ObjectDebugger.DebugSpecialCallback, stack)
        ...
        class DebugRefreshVarCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.ObjectDebugger.DebugRefreshVarCallback, stack)
        ...
        class DebugStepCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.ObjectDebugger.DebugStepCallback, stack)
        ...
        class RunCallback:
            ...
            def start(stack):
                ...
                return Wye.codeFrame(WyeUI.ObjectDebugger.RunCallback, stack)
