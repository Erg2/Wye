...
class TestMyWyeLib:
  def _build():
    WyeCore.Utils.buildLib(TestMyWyeLib)
  canSave = True  # all verbs can be saved with the library
  class TestMyWyeLib_rt:
   pass #1
...
frame.vars.posAngle[0] = angle
    ...
    def _build(rowRef):
        ...
        rowIxRef = [0]
        return WyeCore.Utils.buildParallelText('TestMyWyeLib', 'testParallelFish', TestMyWyeLib.testParallelFish.codeDescr, TestMyWyeLib.testParallelFish)

    def start(stack):
        return TestMyWyeLib.TestMyWyeLib_rt.testParallelFish_start_rt(stack)
