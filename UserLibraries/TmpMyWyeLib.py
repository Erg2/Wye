...
class TmpMyWyeLib:
  def _build():
    WyeCore.Utils.buildLib(TmpMyWyeLib)
  canSave = True  # all verbs can be saved with the library
  class TmpMyWyeLib_rt:
   pass #1
    ...
    codeDescr =        (
        ("loaderStream",
          (
            ...
            ("WyeCore.libs.TestLib.PlaceHolder",),
            ("Label","Done"))),
        ("setAngleStream",
          (
            ("Label","Repeat"),
...
frame.vars.posAngle[0] = angle
    ...
    def _build(rowRef):
        ...
        rowIxRef = [0]
        return WyeCore.Utils.buildParallelText('TmpMyWyeLib', 'testParallelFish', TmpMyWyeLib.testParallelFish.codeDescr, TmpMyWyeLib.testParallelFish)

    def start(stack):
        return TmpMyWyeLib.TmpMyWyeLib_rt.testParallelFish_start_rt(stack)
